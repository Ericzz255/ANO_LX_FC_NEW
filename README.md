...
| 层级 | 技术与实现 |
|---|---|
| 飞控硬件 | Keil 工程目标为 STM32F407VGTx，Cortex-M4F，168 MHz；基于凌霄飞控平台进行二次开发 |
| 机载感知 | N10P 激光雷达、Linux 机载计算平台 SLAM、匿名光流与激光测距、SBUS 遥控器 |
| 嵌入式开发 | Embedded C、CMSIS、STM32F4 标准外设库、裸机协作式多频调度器 |
| 开发工具 | Keil MDK-ARM / µVision 5、ARM Compiler 5，工程入口为 `ProjectSTM32F407/ANO_LX_STM32F407.uvprojx` |
| 定位融合 | 10 Hz SLAM 绝对位置 + 光流速度低权重辅助；X/Y 分轴常速度模型与协方差更新 |
| 运动控制 | 50 Hz 水平位置 PD 外环、高度 PI 外环、低通滤波、死区、积分限幅、速度限幅和变化率限制 |
| 飞控接口 | 凌霄 0x32 外部位置、0x33 外部速度、0x34 测距和 0x41 实时速度控制帧 |
| 任务逻辑 | 有限状态机、航点管理、一次性启动锁存、抛投控制、返航与异常降落 |
| 通信链路 | USART1/2/3/4/5、自定义二进制协议、CRC16-CCITT-FALSE、序号、超时和接收统计 |
| 可观测性 | Linux 地面站 22 字节遥测帧、匿名上位机 USERDATA、任务步骤和故障状态输出 |
| 安全机制 | SLAM 上电资格检查、跳变/断流检测、航点与任务超时、地理围栏、遥控器使能和人工解锁 |

### 飞控端的职责边界

这个工程采用的是**级联控制**思路，而不是重新编写凌霄飞控的全部底层算法：

- 凌霄基础飞控负责惯性传感器处理、姿态稳定、速度内环和电机执行等底层实时控制。
- 本工程新增外部定位接入、位置与高度外环、任务状态机、空地通信、遥测和安全门。
- 外环只生成 X/Y/Z 方向的期望速度，再通过 0x41 实时控制帧交给凌霄内环执行，因此任务逻辑与底层姿态控制保持解耦。
- SLAM 位置和由 SLAM 差分得到的速度还会分别通过 0x32、0x33 帧送入凌霄飞控，形成统一的外部传感器输入链路。

简历中更准确的说法是“**基于凌霄飞控进行 STM32F407 二次开发，设计外部定位、位置/高度外环和自主任务层**”，不宜表述为“从零开发完整飞控”。

### 飞控端控制链路

```mermaid
flowchart LR
    mission_target["任务状态机\n航点 / 高度目标"] --> horizontal["水平位置 PD 外环\n50 Hz"]
    mission_target --> height["高度 PI 外环\n50 Hz"]

    slam["N10P + Linux SLAM\n绝对位置 10 Hz"] --> parser["USART3 帧解析\n长度 / CRC / 字节序"]
    parser --> qualify["SLAM 资格检查\n稳定 / 跳变 / 超时"]
    qualify --> fusion["X/Y 位置融合\n常速度模型 50 Hz"]
    flow["光流速度\n质量门限"] --> fusion
    fusion --> horizontal

    range["激光测距"] --> filter["高度低通滤波"]
    filter --> height

    horizontal --> velocity["X/Y 期望速度"]
    height --> velocity
    velocity --> frame41["0x41 实时控制帧"]
    frame41 --> inner["凌霄速度 / 姿态内环"]
    inner --> motors["电机与飞行器"]
```

### 飞控端关键实现

1. **裸机实时调度**：`main()` 完成硬件初始化后持续运行协作式调度器。任务表预留 1000/500/200/100/50/20/2 Hz 周期，其中自主任务、车机接收和位置融合运行在 50 Hz，Linux 遥测运行在 20 Hz，USERDATA 诊断运行在 10 Hz；本工程未使用 FreeRTOS。
2. **SLAM 接入与有效性管理**：USART3 逐字节状态机解析 `45 53 05` 坐标帧，在长度、CRC16 和大小端检查全部通过后才发布数据。起飞前要求位置连续稳定 3 s；帧间隔超过 300 ms、单帧跳变超过 20 cm 或初始化窗口漂移超过 10 cm 时重新资格检查。
3. **二维位置融合**：X/Y 两轴分别维护位置、速度和 2×2 协方差，以常速度模型按 50 Hz 预测，用 10 Hz SLAM 更新绝对位置；光流速度只有在链路、状态、质量和速度门限均有效时才以较低权重参与更新。
4. **水平位置外环**：采用 `位置误差 → PD → 速度目标` 的串级控制结构，X/Y 独立调参；加入 5 cm 死区、20 cm/s 输出限幅、5 cm/s 最小有效速度和每周期 4 cm/s 变化率限制，降低小范围抖动和指令突变。
5. **高度外环**：激光测距先经过一阶低通滤波，再由 PI 控制器生成 Z 轴速度目标；包含 3 cm 死区、25 cm/s 限幅、积分限幅和 300 ms 传感器超时处理，避免直接控制油门。
6. **自主任务状态机**：将等待启动、SLAM 就绪、遥控解锁、起飞稳定、航点飞行、抛投、返航和降落拆成显式状态，并为航点、全航段和返航分别设置超时与故障转移。
7. **多串口协议工程**：USART1 接收小车 X/Y 与 `TAKEOFF_FLAG`，USART3 接收 SLAM，USART4 接收光流/测距，UART5 与凌霄 IMU 交互，USART2 向 Linux 地面站发送遥测。协议层区分“链路在线”“数据有效”和“任务许可”，并提供 CRC 错误、帧尾错误和字节间超时统计。
8. **安全与可观测性**：把 SLAM 新鲜度、控制故障、航点索引、目标坐标、车机状态和任务步骤输出到 USERDATA/地面站；保留 CH6 安全使能和人工解锁，失去可靠定位时停止水平输出并进入受控降落路径。
...
以下内容适合作为项目经历初稿，但应只保留与你本人实际分工一致、并且能够在面试中结合代码解释的部分。

**项目名称：** 2026 全国大学生电子设计竞赛 D 题——空地协同无人机飞控系统

**技术关键词：** STM32F407、Embedded C、Keil MDK、裸机调度、SLAM、光流、卡尔曼滤波、PD/PI、有限状态机、UART、CRC16

- 基于凌霄 STM32F407 飞控开展二次开发，设计 50 Hz 自主任务与运动控制层，实现 SLAM 定位接入、固定航点飞行、抛投、返航和自动降落流程。
- 构建 SLAM 主导、光流速度低权重辅助的二维位置融合模块，采用分轴常速度模型维护位置/速度状态，并通过数据新鲜度、质量门限和跳变检测拒绝异常观测。
- 设计水平位置 PD 外环和激光高度 PI 外环，将位置/高度误差转换为速度指令，通过凌霄实时控制协议接入底层速度与姿态内环。
- 设计小车、机载 Linux、飞控与地面站之间的多串口二进制协议，完成 CRC16、序号、超时、链路状态和有效性管理，提高系统联调可观测性。
- 使用有限状态机组织一键启动、解锁门控、起飞稳定、航点超时、地理围栏和定位丢失降落等逻辑，避免通信异常直接触发危险动作。

面试时应主动说明：当前仓库已经完成固定航线任务闭环，但动态跟车、运动平台降落和二次起飞仍是后续工作；编译通过与源码存在也不等于所有功能均已完成实物验证。
