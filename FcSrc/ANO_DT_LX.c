...
static u8 send_buffer[50]; //发送数据缓存
_dt_st dt;
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
	...
	case 0x40: //遥控数据帧
	{
		...
		/*
		 * 模式2会直接使用0x40中的原始CH1/CH2/CH3进行速度控制。
		 * 在发送给飞控核心前应用+-100中位死区，避免遥控器中位
		 * 偏差绕过RC_Data_Task中的死区，持续产生水平或垂直移动指令。
		...
		if (fc_sta.fc_mode_sta == 2)
		{
			...
			/*
			 * 自动任务中禁止CH3产生手动上升指令：
			 * 1400以上全部按中位1500发送，低于1400仍保留下降控制。
			 */
			if (rc_to_fc.st_data.ch_[ch_3_thr] >= 1400)
			{
				rc_to_fc.st_data.ch_[ch_3_thr] = 1500;
			}
		}
